function [] = preprocessDataPPA(loc, numTrain, numTest, matPATH, fileType, numFolders)
%preprocessing Code
%Davis Liang, 6/16/15
...
%% intializing
close all;
display(['Initiating Rectilinear Preprocessor...']);
...
%64*5*8 
f_filtered_normalized_vector_dwsp_allsub_train = zeros(imageSize*numScales*numOrientations*angles, total_train);
f_filtered_normalized_vector_dwsp_allsub_test = zeros(imageSize*numScales*numOrientations*angles, total_test);
...
for i=1:length(files)
    ...
    for scale=1:size(sizeMod,2)
        f_filtered_normalized_vector_dwsp((scale-1)*dwsp*dwsp*size(phi,2)*size(theta,2)+1:(scale)*dwsp*dwsp*size(theta,2)*size(phi,2)) = f_filtered_normalized_dwsp{scale}(:);
    end
    

    %assemble the test and the training set.
    if train_indice(i) == 0
        f_filtered_normalized_vector_dwsp_test(:,test_i) = f_filtered_normalized_vector_dwsp(:);
        test_i = test_i + 1;
    else
        f_filtered_normalized_vector_dwsp_train(:,train_i) = f_filtered_normalized_vector_dwsp(:);
        train_i = train_i + 1;
    end
    clear f_filtered_normalized_vector_dwsp
end
    
%% zscore (use training set mean and std to calculate zscore for the test set).
STD = std(f_filtered_normalized_vector_dwsp_train,[],2);
MEAN = mean(f_filtered_normalized_vector_dwsp_train,2);
f_filtered_normalized_vector_dwsp_allsub_train = bsxfun(@minus, f_filtered_normalized_vector_dwsp_train, MEAN);
f_filtered_normalized_vector_dwsp_allsub_train = bsxfun(@rdivide, f_filtered_normalized_vector_dwsp_train, STD + 0.1);
f_filtered_normalized_vector_dwsp_allsub_test = bsxfun(@minus, f_filtered_normalized_vector_dwsp_test, MEAN);
f_filtered_normalized_vector_dwsp_allsub_test = bsxfun(@rdivide, f_filtered_normalized_vector_dwsp_test, STD + 0.1);
    
    


data.train = f_filtered_normalized_vector_dwsp_allsub_train;
data.test = f_filtered_normalized_vector_dwsp_allsub_test;
...
for s=1:size(k,2) 
    scale_all=f_filtered_normalized_vector_dwsp_allsub_train((s-1)*dwsp*dwsp*size(theta,2)*size(phi,2)+1:s*dwsp*dwsp*size(theta,2)*size(phi,2),:);
    scale_all_test=f_filtered_normalized_vector_dwsp_allsub_test((s-1)*dwsp*dwsp*size(phi,2)*size(theta,2)+1:s*dwsp*dwsp*size(theta,2)*size(phi,2),:);
...
for folders=1:numFolders
    for images=1:numTrain
        organizedData.train{folders}(:,images) = data.train(:,images+(counter*numTrain));
        organizedData.trainlabel{folders}(images) = data.train_label(images);
    end

    for images=1:numTest
        organizedData.test{folders}(:,images) = data.test(:,images+(counter*numTest));
        organizedData.testlabel{folders}(images) = data.test_label(images);
    end
    counter = counter + 1;
end

organizedData.MEAN = MEAN;
organizedData.STD = STD;
organizedData.cov_scale = cov_scale;
organizedData.mean_images = mean_images;
organizedData.total_train = total_train;
organizedData.mean_subst = mean_subst;
